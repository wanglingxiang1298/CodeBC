...
contract MerkleProofVerifier {
    ...
    function processProof(bytes32[] memory proofs, bytes32 leaf) public pure returns (bytes32) {
    ...
    function multiProofVerify(bytes32[] memory proofs, bool[] memory proofFlags, bytes32[] memory rootHashes, bytes32 leaf) public pure returns (bool) {
        return MerkleProof.multiProofVerify(proofs, proofFlags, rootHashes, leaf);
    }
    function multiProofVerifyCalldata(bytes32[] memory proofs, bool[] memory proofFlags, bytes32[] memory rootHashes, bytes32 leaf) public view returns (bool) {
        return MerkleProof.multiProofVerify(proofs, proofFlags, rootHashes, leaf);
    }
    function processMultiProof(bytes32[] memory proofs, bool[] memory proofFlags, bytes32[] memory leafHashes) public pure returns (bytes32) {
        return MerkleProof.processMultiProof(proofs, proofFlags, leafHashes);
    }
    function processMultiProofCalldata(bytes32[] memory proofs, bool[] memory proofFlags, bytes32[] memory leafHashes) public view returns (bytes32) {
        return MerkleProof.processMultiProof(proofs, proofFlags, leafHashes);
    }
